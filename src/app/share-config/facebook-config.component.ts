...
import {
  generateNanoid, customizePaginatorTxt, moveElement, propertyFields, translateType, isDebug, LogUtil,
} from '../../utils/utils';
...
@Component({
  selector: 'facebook-config',
  templateUrl: './facebook-config.component.html',
  styleUrls: [],
})
export class FacebookConfigComponent extends ShareConfigComponent { // eslint-disable-line import/prefer-default-export
  ...
  async save() {
    ...
    const config = this.formData.value;
    let payload = null;
    payload = {
      atMaps: [
        {
