...
@Component({
  selector: 'wechat-config',
  templateUrl: './wechat-config.component.html',
  styleUrls: [],
})
export class WeChatConfigComponent extends ShareConfigComponent { // eslint-disable-line import/prefer-default-export
  ...
  async save() {
    ...
    const WeChatConfig = this.formData.value;
    let payload = null;
    payload = {
      atMaps: [
        {
