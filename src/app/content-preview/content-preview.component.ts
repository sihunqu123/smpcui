...
import {
  APPConfigModel, AuthoringTemplateModel, MappingModel, paginateModelChild, ContentModel, ToBeShareModel, ToPreviewModel, PreviewModel, ATConfig,
} from '../../model/index';
...
export class ContentPreviewComponent extends CommonParent implements OnInit, OnChanges { // eslint-disable-line import/prefer-default-export
  ...
  render(): any[] {
    const renderVal = this.previewItems.filter((item) => item.fieldsVal).map((currentItem) => {
      ...
      switch (mediaType) {
        case 'image': {
          const value = imageFields?.map((imageField: string) => fieldsVal[imageField]).filter((_item) => _item);
